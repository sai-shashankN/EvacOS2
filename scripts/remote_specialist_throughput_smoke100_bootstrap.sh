...
REPO_TGZ="/root/evacos2_source_smoke100.tgz"
WORKDIR="/workspace/EvacOS2"
RUN_NAME="remote-unsloth-3b-${DISASTER_FAMILY}-floor-specialist-throughput-smoke-100"
CONFIG="training/config.remote-unsloth-3b-${DISASTER_FAMILY}-floor-specialist-throughput-smoke-100.yaml"
METRICS="outputs/training/${RUN_NAME}-metrics.csv"
ARTIFACT_DIR="/root/evacos2_${DISASTER_FAMILY}_smoke100_artifacts"
ARTIFACT_TGZ="$ARTIFACT_DIR/${DISASTER_FAMILY}_smoke100_artifacts.tgz"
JSONL_DIR="outputs/logs/${RUN_NAME}"
CHECKPOINT_DIR="outputs/training/${RUN_NAME}"
REPORT="$ARTIFACT_DIR/${DISASTER_FAMILY}_smoke100_report.json"
...
if [[ -f "$REPO_TGZ" ]]; then
  ...
  echo "Missing repo upload: expected $REPO_TGZ" >&2
  exit 2
fi
...
python -m pytest tests/test_config_schema.py tests/test_specialist_configs.py tests/test_check_grpo_contrast.py -q --basetemp .pytest_tmp_remote_smoke100
...
tar -C "$ARTIFACT_DIR" -czf "/root/${DISASTER_FAMILY}_smoke100_artifacts.tmp.tgz" . 2>"/tmp/${DISASTER_FAMILY}_smoke100_tar_warnings.log" || true
mv "/root/${DISASTER_FAMILY}_smoke100_artifacts.tmp.tgz" "$ARTIFACT_TGZ" 2>/dev/null || true
cp "/tmp/${DISASTER_FAMILY}_smoke100_tar_warnings.log" "$ARTIFACT_DIR/" || true
exit "$TRAIN_EXIT"
